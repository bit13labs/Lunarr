...
namespace NzbDrone.Common.EnvironmentInfo
{
    ...
    public class AppFolderFactory : IAppFolderFactory
    {
        ...
        public void Register()
        {
            ...
            {
                throw new LunarrStartupException("Cannot create AppFolder, Access to the path {0} is denied", _appFolderInfo.AppDataFolder);
            }
            ...
            {
                throw new LunarrStartupException("AppFolder {0} is not writable", _appFolderInfo.AppDataFolder);
            }
        ...
        private void MigrateAppDataFolder()
        {
            try
            {
                ...
                // Exit if a lunarr.db already exists
                if (_diskProvider.FileExists(_appFolderInfo.GetDatabase()))
                {
                    return;
                }
            ...
            catch (Exception ex)
            {
                ...
                throw new LunarrStartupException("Unable to migrate DB from nzbdrone.db to {0}. Migrate manually", _appFolderInfo.GetDatabase());
            }
        }
        ...
        private void RemovePidFile()
        {
            ...
            {
                _diskProvider.DeleteFile(Path.Combine(_appFolderInfo.AppDataFolder, "lunarr.pid"));
            }
        }
    }
}
