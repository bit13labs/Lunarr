        ...
        private ValidationFailure TestConnectionAndVersion()
        {
            try
            {
                ...
                if (rawVersion.Equals("develop", StringComparison.InvariantCultureIgnoreCase))
                {
                    ...
                    {
                        IsWarning = true,
                        DetailedDescription = "Lunarr may not be able to support new features added to SABnzbd when running develop versions."
                    };
                }
        ...
        private ValidationFailure TestGlobalConfig()
        {
            ...
            if (config.Misc.pre_check && !HasVersion(1, 1))
            {
                return new NzbDroneValidationFailure("", "Disable 'Check before download' option in SABnzbd")
                {
                    ...
                    DetailedDescription = "Using Check before download affects Lunarr ability to track new downloads. Also SABnzbd recommends 'Abort jobs that cannot be completed' instead since it's more effective."
                };
            }
        ...
        private ValidationFailure TestCategory()
        {
            ...
            if (category != null)
            {
                if (category.Dir.EndsWith("*"))
                {
                    return new NzbDroneValidationFailure("MovieCategory", "Enable Job folders")
                    {
                        ...
                        DetailedDescription = "Lunarr prefers each download to have a separate folder. With * appended to the Folder/Path SABnzbd will not create these job folders. Go to SABnzbd to fix it."
                    };
                }
            }
            else
            ...
            if (config.Misc.enable_tv_sorting && ContainsCategory(config.Misc.tv_categories, Settings.MovieCategory))
            {
                return new NzbDroneValidationFailure("MovieCategory", "Disable TV Sorting")
                {
                    ...
                    DetailedDescription = "You must disable SABnzbd TV Sorting for the category Lunarr uses to prevent import issues. Go to SABnzbd to fix it."
                };
            }
            ...
            if (config.Misc.enable_movie_sorting && ContainsCategory(config.Misc.movie_categories, Settings.MovieCategory))
            {
                ...
                {
                    InfoLink = _proxy.GetBaseUrl(Settings, "config/sorting/"),
                    DetailedDescription = "You must disable SABnzbd Movie Sorting for the category Lunarr uses to prevent import issues. Go to SABnzbd to fix it."
                };
            }
            ...
            if (config.Misc.enable_date_sorting && ContainsCategory(config.Misc.date_categories, Settings.MovieCategory))
            {
                ...
                {
                    InfoLink = _proxy.GetBaseUrl(Settings, "config/sorting/"),
                    DetailedDescription = "You must disable SABnzbd Date Sorting for the category Lunarr uses to prevent import issues. Go to SABnzbd to fix it."
                };
            }
