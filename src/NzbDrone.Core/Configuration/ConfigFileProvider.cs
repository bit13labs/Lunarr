...
namespace NzbDrone.Core.Configuration
{
    ...
    public class ConfigFileProvider : IConfigFileProvider
    {
        ...
        public string PostgresMainDb => _postgresOptions?.MainDb ?? GetValue("PostgresMainDb", "lunarr-main", persist: false);
        public string PostgresLogDb => _postgresOptions?.LogDb ?? GetValue("PostgresLogDb", "lunarr-log", persist: false);
        ...
        public XDocument LoadConfigFile()
        {
            try
            {
                lock (Mutex)
                {
                    if (_diskProvider.FileExists(_configFile))
                    {
                        ...
                        {
                            throw new InvalidConfigFileException($"{_configFile} is empty. Please delete the config file and Lunarr will recreate it.");
                        }

                        if (contents.All(char.IsControl))
                        {
                            throw new InvalidConfigFileException($"{_configFile} is corrupt. Please delete the config file and Lunarr will recreate it.");
                        }
            ...
            {
                throw new InvalidConfigFileException($"{_configFile} is corrupt is invalid. Please delete the config file and Lunarr will recreate it.", ex);
            }
        }
